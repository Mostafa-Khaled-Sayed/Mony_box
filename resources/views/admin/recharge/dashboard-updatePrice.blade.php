...
تعديل السعر 
        ...
        <form action="{{route('packageprice.update',$pricedate->id)}}" method="post"   class="dashboard-new">
            @csrf
         <div class="row">
       
<input type="hidden" name=id value="{{$pricedate->id}}" >
           <h4 class="sub-title col-12 my-4">إضافة رصيد </h4>
           <div class="form-group col-md-6">
             <label class="col-form-label"> الأسم</label>
               <input type="text" required name="name" value="$pricedate->name" class="form-control">
           </div>
           <div class="form-group col-md-6">
             ...
             <select required name="charge"  class="form-control">
                 ...
                 @if($pricedate->company_incountrie_id===$value->id)
...
@endif
           ...
           <div class="form-group col-md-6">
             ...
             <select name="status" class="form-control">
               <option value="{{$pricedate->$pricedate?1:0}}">{{$pricedate->$pricedate?"مفعل":"غير مفعل"}}</option>
               <option value="{{!$pricedate->$pricedate?1:0}}">{{!$pricedate->$pricedate?"مفعل":"غير مفعل"}}</option>
           ...
           <div class="form-group col-md-6">
             <label class="col-form-label"> الوصف  </label>
               <input name="description" value="$pricedate->description" type="text" class="form-control">
           </div>
           <div class="form-group col-md-6">
             <label class="col-form-label"> السعر </label>
               <input name="price" value="{{$pricedate->price}}" type="text" class="form-control">
           </div>
           <div class="form-group col-md-6">
             <label class="col-form-label"> الصلاحية  </label>
               <input type="text"name="validate" value={{$pricedate->validate}}  class="form-control">
           </div>
           <div class="form-group col-md-6">
             <label class="col-form-label"> رمز الرصيد </label>
               <input type="text" name="identified_code" value="{{$pricedate->identified_code}}" required class="form-control">
           </div>
           <div class="form-group col-md-6">
             <label class="col-form-label">  رمز الكود الموحد للربط</label>
               <input name="simble_reacherage" value="{{$pricedate->simble_reacherage}}" type="text" class="form-control">
           </div>
