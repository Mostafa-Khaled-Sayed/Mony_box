...
@section('main')
 ...
 @endforeach
  ...
  <div class="modal slide-bottom" id="serviceDetalies" tabindex="-1" aria-labelledby="staticBackdropLabel"
    aria-hidden="true">
    <div class="modal-dialog assets-card container">
      <div class="d-flex align-items-center justify-content-between pb-2">
      ...
      <div class="modal-content ">
        ...
        <a class="follow-btn text-center">متابعة</a>
      </div>
    </div>
  </div>
                ...
                if (company_id) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $('select[name="companyincome"]').append('<option selected disabled>مزود الخدمة</option>');
                            $.each(data, function (key, value) {

                                $('select[name="companyincome"]').append('<option value="' + key + '">' + value + '</option>');
                            });
                        },
                    });
                } else {
            ...
            $('select[name="companyincome"]').on('change', function () {
                ...
                if (company_id) {
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        success: function (data) {
console.log(data.package_price);
                             $('div.pricesdata').empty();

                             $.each(data.package_price, function ({company,company_incountrie_id,created_at,description,id,identified_code,image,imagebackground,name,price,simble_reacherage,status,updated_at,validate}) {

                               $('div.pricesdata').append(`
                <div class="Filling-amount">
              <a href="#" data-toggle="modal" data-target="#serviceDetalies">
                <span>${this.price} EGP</span>
                <span>0.14 $</span>
              </a>
            </div>
